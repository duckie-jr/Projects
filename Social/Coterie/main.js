...
function showAppScreen() {
  ...
  roomBrowserScreenEl?.classList.add("hidden");
...
const SESSION_KEY_CHAT_PREFIX = "coterie_chat_";
...
function setLobbyStatus(text, isError = false) {
  ...
  // Mirror into room browser status if it exists
  if (rbStatusEl) {
    rbStatusEl.textContent = text;
    rbStatusEl?.classList.toggle("error", isError);
  }
}
...
function resetLobbyButtons() {
  ...
  if (rbJoinBtnEl)    rbJoinBtnEl.disabled     = false;
  if (rbRefreshBtnEl) rbRefreshBtnEl.disabled  = false;
}
